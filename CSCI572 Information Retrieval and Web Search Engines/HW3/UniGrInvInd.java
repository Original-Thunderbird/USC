...
public class UniGrInvInd {

    public static class UniGrInvIndMapper extends Mapper<Object, Text, Text, Text> {

        private final Text id = new Text(), unigram = new Text();

        @Override
        protected void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            String[] doc = value.toString().split("\\t", 2);
            String word, content = doc[1].toLowerCase().replaceAll("[^a-z]+", " ");
            ...
            while (tokenizer.hasMoreTokens()) {
                ...
                if (word.trim().isEmpty() == false) {
                    unigram.set(word);
                    context.write(unigram, id);
                }
            }
        }
    }

    public static class UniGrInvIndReducer extends Reducer<Text, Text, Text, Text> {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        job.setJarByClass(UniGrInvInd.class);
        job.setMapperClass(UniGrInvIndMapper.class);
        job.setReducerClass(UniGrInvIndReducer.class);
